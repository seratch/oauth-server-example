...
public class TwoLeggedOAuthTest {
    ...
    public void get() throws Exception {
        ...
        WebResource resource = client.resource(new URI("http://localhost:8080/2legged/"));

        OAuthParameters params = new OAuthParameters();
        params.signatureMethod("HMAC-SHA1").consumerKey("CONSUMER_KEY");
    ...
    public void get_401() throws Exception {
        ...
        params.signatureMethod("HMAC-SHA1").consumerKey("CONSUMER_KEY");
        OAuthSecrets secrets = new OAuthSecrets();
        secrets.consumerSecret("CONSUMER_SECRETXXX");
        OAuthClientFilter filter = new OAuthClientFilter(client.getProviders(), params, secrets);
        resource.addFilter(filter);
        System.out.println(resource.get(String.class));
    }
    ...
    public void post_200() throws Exception {
        ...
        ClientConfig config = new DefaultClientConfig();
        Client client = Client.create(config);
        WebResource resource = client.resource(new URI("http://localhost:8080/2legged/post/submit"));
